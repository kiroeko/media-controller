...
旋转方向、模式切换和按键动作已在整机上验证。切歌模式在成功排入一次切歌动作后进入 300 ms 冷却期，期间继续读取旋钮，但忽略正反方向的后续旋转；到期后新的卡点才会再次切歌。冷却期不会因被忽略的旋转而延长，也不影响音量和按键操作。这个新增的冷却行为还需刷入新固件上板确认。LED 自锁开关单独通电时，测得灯灭时 `SIG` 对 `GND` 稳定约为 0 V、灯亮时约为 3.3 V，与固件设定的高电平有效极性一致。
