...
EC11 按键的手势由 `src/channel/switch_channel.cpp` 检测：**短按**在松开时判定（所以播放/暂停现在在**松开时**才触发）、**长按** ≥700ms 越过阈值即发、**双击**为两次按压间隔 ≤300ms。当前绑定：**短按 = 播放/暂停**、**长按 = 静音切换**（`HID_USAGE_CONSUMER_MUTE` 0xE2；Windows 侧是 toggle，一次长按交替静音/取消，固件不需要第二个 usage）；双击已检测、未绑定。双击检测默认关闭：开启后每次短按都要等 300ms 间隔窗结束才能发出，绑定双击动作那天再打开（`RotationDevice` 构造处第四个参数）并接受这个延迟。
